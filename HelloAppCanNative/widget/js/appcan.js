    ...
    var logs = function (obj) {
        try {
            if (window.uexLog) {
                //2000字符作为分割，因为插件有控制
                var flag = 2000;
                for (var i = 0, len = obj.length / flag; i < len; i++) {
                    var logstr = obj.substr(i * flag, flag);
                    window.uexLog && uexLog.sendLog(logstr);
                }
            } else {
...
appcan && appcan.define('file', function ($, exports, module) {
    ...
    function getRealPath(filePath, callback) {
        ...
        if (isSML) {
            //TODO:模拟器调试
            setTimeout(function () {
                uexFileMgr.cbGetFileRealPath(0, 0, 'wgtRes');
            }, 1200);
        }
    }
...
window.appcan && appcan.define('window', function ($, exports, module) {
    ...
    /*
     ...
     1:颜色弹动效果
     2:设置图片弹动
     @param Function startPullCall 开始下拉的回调
    ...
    function setBounce(bounceType, startPullCall, downEndCall, upEndCall, color, imgSettings) {
        var argObj = null;
        var json = {
            "imagePath": "res://jiantou.png",
            "textColor": "#530606",
            "pullToReloadText": tools.getString("STR_DRAG_FLASH"),
            "releaseToReloadText": tools.getString("STR_RELEASE_FLASH"),
            "loadingText": tools.getString("STR_LOAD_MORE"),
        ...
        if (arguments.length === 1 && appcan.isPlainObject(bounceType)) {
            ...
            imgSettings = argObj['imgSettings'] || json
        }
        color = color || 'rgba(255,255,255,0)';
        imgSettings = imgSettings || json
        var startBounce = 1;
        ...
        if (startPullCall || downEndCall || upEndCall) {
            ...
            for (var i in bounceType) {
                uexWindow.setBounceParams(bounceType[i], JSON.stringify(imgSettings), "donghang");
     ...
     */
    function publishForJson(channelId, msg) {
        if (arguments.length === 1 && appcan.isPlainObject(channelId)) {
            msg = channelId['msg'];
            channelId = channelId['channelId'];
        }
        ...
        uexWindow.publishChannelNotificationForJson(channelId, msg);
    }
...
window.appcan && appcan.define('frame', function ($, exports, module) {
    ...
    var appcanFrame = module.exports = {
        ...
        publishForJson: appWin.publishForJson,
        ...
        open: function (id, url, left, top, name, index, change, extraInfo) {
            ...
            if (appcan.isArray(url)) {
                ...
                appWin.openMultiPopover(name || id,
                    url, 0, left, top, width, height, change || function () {
...
appcan && appcan.define('download', function ($, exports, module) {
    ...
    function processDownloadQueue(optId, fileSize, percent, status) {
        ...
        if (status == 0) {//DownLoading
            ...
            appcan.logs('uexDownloaderMgr.onStatus undifinde optId=' + optId + ',status=' + status);
        }
    ...
    function createDownloader(optId, callback) {
        ...
        if (isSML) {
            //TODO:模拟器调试
            uexDownloaderMgr.cbCreateDownloader(optId, 2, 0);
        } else {
    ...
    function download(optId, url, savePath, breakPoint, downloadCall, successCall, errorCall, cancelCall) {
        ...
        if (isSML) {
            ...
            for (var i = 0; i <= 100; i = i + 20) {
                if (i == 100) {
                    //下载完成
                    setTimeout('uexDownloaderMgr.onStatus(' + optId + ', 1024, ' + i + ', 1)', 1 * i);
                } else {
                    //下载中
                    setTimeout('uexDownloaderMgr.onStatus(' + optId + ', 1024, ' + i + ', 0)', 1 * i);
                }
            }
        } else {
    ...
    function queueInfo() {
        var queueInfo = {
            createDownloaderQueue: createDownloaderQueue,
...
appcan.define("icache", function ($, exports, module) {
    var opid = 1000;
    var CACHE_PATH = "exterbox://icache/";
    ...
    function iCache(option) {
        ...
        appcan.file.getRealPath("exterbox://", function (err, data, dataType, optId) {
            self.realpath = data;
    ...
    iCache.prototype = {
        ...
        _success: function (fpath, session) {
            ...
            var fpathOld = fpath;
            // fpath = fpath + '?t=' + new Date().getTime();
            ...
            if (session.success) {
                ...
                switch (session.dom[0].tagName.toLowerCase()) {
                    case "img":
                        if (session.dom.attr("src") != fpath) {
                            if (location.href.indexOf('http') == 0 || isSML) {
                                //TODO:浏览器调试
                                var tpath = session.dom.attr("data-original");
                                session.dom.attr("src", tpath);
                            } else {
                        ...
                        break;
                    default:
                        if (fpath && session.dom.css("background-image") != fpath) {
                            if (location.href.indexOf('http') == 0 || isSML) {
                                //TODO:浏览器调试
                                var tpath = session.dom.attr("data-original");
                                session.dom.css("background-image", "url(" + tpath + ") !important");
                            } else {
                                session.dom.css("background-image", "url(" + fpath + ") !important");
                            }
                        }
                        break;
                }
            ...
            ;
            var runningS = self.running.valueOf();
            if (runningS) {
                var index = self.getIndex(runningS, session);
                if (index != undefined) {
                    //删除已下载的
        ...
        //获取已下载的在running数组的位置
        getIndex: function (list, session) {
            var index = undefined;
        ...
        _next: function () {
            ...
            if (!session)
                return;
            /*查询当前下载的路径是否已在下载任务进行中，否：进入下载，是：继续等待*/
        ...
        _download: function (session) {
            var self = this;
            console.log('session.url server = ' + session.url + ' length = ' + session.url.length);
            console.log('session.url server = ' + $.trim(session.url) + ' length = ' + $.trim(session.url).length);
            var fid = appcan.crypto.md5($.trim(session.url));
            var fpath = self.realpath + "/icache/" + fid + (session.ext ? session.ext : "");
            session.fpath = fpath;
            self.on("DLS" + session.id, function (data) {
                switch (parseInt(data.status)) {
                    case 0:
                        ...
                        break;
                    case 1:
                        self._success(session.fpath, session);
                        break;
                    default:
                        self._fail(session);
                        break;
            ...
            self.once("FS" + session.id, function (data) {
                if (parseInt(data)) {
                    ...
                    session.dlId = appcan.download.create(null, function (err, data, dataType, optId) {
                        if (!err) {
                            session.header && appcan.download.setHeaders(optId, session.header);
                            appcan.download.run(optId, $.trim(session.url), fpath, '0', downloadCB, downloadCB, downloadCB, downloadCB);
                        } else {
            ...
            appcan.file.exists(session.fpath, function (err, data, dataType, optId) {
                self.emit("FS" + session.id, self, data);
            });
        },
        query: function (session) {
            var self = this;
            for (var i = 0, l = self.running.length; i < l; i++) {
                if (self.running[i].url == session.url) {
                    return false;
                }
            }
            return true;
        ...
        queueInfo: function () {
            var self = this;
            var info = {
                waiting: self.waiting.length,
                running: self.running.length
            };
            return info;
        }
    };
...
;(function (factory) {
    ...
    var w = window,
        ...
        defaultOptions = {
            ...
            placeholder_real_img: 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAAAXNSR0IArs4c6QAAAARnQU1BAACxjwv8YQUAAAAJcEhZcwAADsQAAA7EAZUrDhsAAAANSURBVBhXYzh8+PB/AAffA0nNPuCLAAAAAElFTkSuQmCC'
    ...
    appcan.ready(function () {
        if (!window.lazyICache) {
            window.lazyICache = appcan.icache({maxtask: 3});
        }
    })
                ...
                $element.one('_lazyload_appear', function () {
                    ...
                    function loadFunc() {
                        ...
                        if (isImg) {
                            ...
                            if (originalSrcset) {
                                if (options.cache)
                                    lazyLoadImage(originalSrcset, $element, options.callBack);
                                else {
                                    $element.attr('srcset', originalSrcset);
                                    options.callBack(originalSrcset, {
                                        dom: $element
                                    })
                                }
                            }
                            if (originalSrc) {
                                if (options.cache) {
                                    lazyLoadImage(originalSrc, $element, options.callBack);
                                } else {
                                    $element.attr('src', originalSrcset);
                                    options.callBack(originalSrc, {
                                        dom: $element
                                    })
                                }
                            }
 ...
 * @param log
 */
function logAlert(log) {
    try {
        if (!isLog) {
            return;
        }
        log = processLog(log);
        appcan.window.alert('alert', log);
    } catch (e) {
        errorDetail(e);
    }
}
...
function errorDetail(e) {
    setTimeout(function(){alert("name: " + e.name + "\nmessage: " + e.message + "\nlineNumber: " + e.lineNumber + "\nfileName: " + e.fileName + "\nstack: " + e.stack);},500)
}
...
function getCurrentHtmlName() {
    var fullHtmlPath = window.location.href;//获得完整的HTML路径
    // log("fullHtmlPath = " + fullHtmlPath);
    var arrayFullHtmlPath = fullHtmlPath.split('/');//把完整的HTML路径用/分割
    var simpleHtmlPath = arrayFullHtmlPath[arrayFullHtmlPath.length - 1];//取最后一个路径即可
    // log("simpleHtmlPath = " + simpleHtmlPath);
    return simpleHtmlPath;
}
 ...
 *
 * @param log 未加工的log
 * @return newLog 加工后的log
 */
function processLog(log) {
    var newLog = "**" + getCurrentHtmlName() + "**" + log;
    return newLog;
}
