...
appcan.extend(function (app, exports, module) {
    ...
    function updateSwitch(obj) {
        ...
        value == 'false' ? obj.removeClass("bg-color-primary") : obj.addClass("bg-color-primary");
    ...
    function switchBtn(selector, css, cb) {
        ...
        for (var i = 0; i < obj.length; i++) {
            ...
            value == 'false' ? $(obj[i]).removeClass(css ? css : "bg-color-primary") : $(obj[i]).addClass(css ? css : "bg-color-primary");
        }
        $(selector).on("tap", function (evt) {
            ...
            value = (value == 'false' ? 'true' : 'false');
            obj.attr("data-checked", value);
            value == 'false' ? obj.removeClass(css ? css : "bg-color-primary") : obj.addClass(css ? css : "bg-color-primary");
    ...
    var ONCLICK_ANIMATION_PROTECTION_TIMES = 500;//按钮点击保护时间

    function touch(className, fun) {

        log("isNavCenterOnClick = " + isButtonOnClick);
        log("NAV_CENTER_ANIMATION_TIMES = " + ONCLICK_ANIMATION_PROTECTION_TIMES);
...
appcan.ready(function () {
    ...
    if (ios7style == '1' && isFullScreen != '1') {
        ...
        $('#scrollBox').addClass('status-bar-iOS');
    }
});
